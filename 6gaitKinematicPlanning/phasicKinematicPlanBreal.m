function kinemPlan = phasicKinematicPlanBreal (feasGeomMap,Ts)
    ...
    % TRAJECTORY OBTAINED IN BODY REAL FRAME!
    ...
    for i = 1:(size(feasGeomMap,2)/6)-1 %number of full stances minus last one
        for j = 1:6 % legs
          for k= 1:3 % x, y, z
            ...
            if strcmp(feasGeomMap((i-1)*6+1).phase,'pInt') && feasGeomMap((i-1)*6+j).st1_sw0 == 0 && k ~= 2   
            ...
            elseif strcmp(feasGeomMap((i-1)*6+1).phase,'hex') && feasGeomMap((i)*6+j).st1_sw0 == 0 && k ~= 2      
                t0 = feasGeomMap((i-1)*6+1).t;
                tf = feasGeomMap((i)*6+1).t;
                p0 = feasGeomMap((i-1)*6+j).BiFootPos(k);
                pf = feasGeomMap((i)*6+j).BiFootPos(k);
